...
export const profiles = pgTable('profiles', {
  ...
  skills: jsonb('skills').$type<{ name: string; selfRating: number; source: 'onboarding' | 'added' | 'inferred' }[]>().default([]),
  ...
  workHistory: jsonb('work_history').$type<{ title: string; company: string; months: number }[]>().default([]),
...
export const savedCareers = pgTable('saved_careers', {
  ...
  salaryRange: jsonb('salary_range').$type<{ entry: number; mid: number; senior: number }>(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
})
...
export const savedResources = pgTable('saved_resources', {
  id: uuid('id').primaryKey().defaultRandom(),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
  resourceType: text('resource_type').$type<'video' | 'article' | 'course' | 'wiki'>().notNull(),
  ...
  status: text('status').$type<'new' | 'in_progress' | 'completed'>().default('new'),
  createdAt: timestamp('created_at').defaultNow().notNull(),
})
